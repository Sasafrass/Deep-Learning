...
class CustomBatchNormAutograd(nn.Module):
  ...
  def forward(self, input):
    ...
    # Check for correctness of shapes
    if input.shape[0] < 2:
      raise ValueError("We didn't get a batch: how would we do batch normalization!?")
    if input.shape[1] != self.gamma.shape[0]:
      raise ValueError("Number of neurons doesn't match up!")
...
class CustomBatchNormManualFunction(torch.autograd.Function):
  ...
  def forward(ctx, input, gamma, beta, eps=1e-5):
    ...
    # Save constants and backward
    ctx.save_for_backward(input, gamma, beta, mean, var, x_norm)
    ctx.constant = eps 
  ...
  def backward(ctx, grad_output):
    ...
    # Retrieve tensors    
    input, gamma, beta, mean, var, x_norm = ctx.saved_tensors

    # Retrieve bool values from tuple
    inputgrad = ctx.needs_input_grad[0]
    gammagrad = ctx.needs_input_grad[1]
    betagrad = ctx.needs_input_grad[2]

    # Calculate partial of x_norm
    dx_norm = grad_output * gamma

    # save batch dimension
    m = input.shape[0]

    # Calculate the total gradients for all saved tensors

    # For input 
    if inputgrad:
      grad_input = 1 / (m * torch.sqrt(var+ctx.constant)) * ( m * dx_norm - torch.sum(dx_norm, dim = 0) - x_norm * torch.sum(dx_norm * x_norm, dim = 0)) 
    else:
      None
